...
async function formCheck(theUserName, thePassword, navigation) {
    try {
        ...
        if (value == null) {
            ...
            Alert.alert("로그인 완료!");
            await AsyncStorage.setItem(__USER, '__' + theUserName);
            navigation.navigate('Menu', { currentUser: '__' + theUserName, });
        }
        else {
            Alert.alert("아이디나 비밀번호가 틀렸습니다!");
        }
    ...
    return (
        <ImageBackground source={require('../assets/images/background1.jpg')} style={{ flex: 1, width: '100%', height: '100%' }}>
            <View style={styles.titles}>
                <Text style={{ textAlign: 'center', color: '#fff', fontSize: 50 }}>Login</Text>
            </View>
            <View style={styles.inputs}>
                ...
                <TouchableOpacity onPress={() => navigation.navigate('Sign')}>
                    <Text style={{ textAlign: 'center', fontSize: 16, color: '#0080ff' }}> Register </Text>
                </TouchableOpacity>
            </View>
